"""Shared prompt-evaluation harness for the *Prompt engineering techniques* section.

This is the course's `PromptEvaluator` (from the downloadable `001_prompting.ipynb`),
extracted into a module so every technique lesson in this section can `import` it
instead of pasting ~400 lines into each notebook.

Adaptations from the course original:
  * `chat()` returns via a `get_text()` helper (scans for the first text block) instead
    of `message.content[0].text`, so it's robust to thinking blocks.
  * `load_dotenv(find_dotenv())` so the repo-root .env loads regardless of cwd.
  * Model id `claude-haiku-4-5-20251001` (matches the rest of the repo). Haiku is required
    here anyway: the harness uses assistant prefilling AND temperature, both of which 400
    on flagship models (Sonnet 5, Opus 4.7/4.8, Fable 5).
"""
